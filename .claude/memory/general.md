...
2026-09-13: on the live prototype-tools site, Python in GeoLibre's JupyterLite notebook reached the host page and got a reply over a same-origin `BroadcastChannel` opened from the Pyodide worker with `js.BroadcastChannel`, tested by headless Chrome with a listener injected into the host page. The Earth Engine panel (Processing menu, `maplibre-gl-earth-engine` 0.4.2 pinned by GeoLibre v1.9.0) sets `window.ee` only inside `initializeEarthEngine`, so `window.ee` is undefined until someone signs in, and a signed-in request through the bridge has not been run. The Python `earthengine-api` does not install in Pyodide, with `google-crc32c` missing a pure wheel and `micropip.install(..., deps=False)` raising an internal error. Matters because any notebook access to Earth Engine has to run through the panel's signed-in JavaScript client rather than a Python client.

2026-09-13: the bridge ran against real signed-in Earth Engine. Seamus pasted the "Connect to Earth Engine through GeoLibre" cell into the live notebook, and `await gee.run("ee.Image('projects/murphys-deforisk/assets/consensus_2019').bandNames()")` returned `['b1']`. The cell lives only in chat and in neither repository. Matters because the notebook-to-panel route is now proven end to end, not only with a stand-in `ee`.

2026-09-13: Earth Engine layers reach GeoLibre's Layers panel from the notebook through `window.__disturbanceCheckNotebook.addTileLayer`, set in `src/notebook/layers.ts` in both repositories, which wraps the plugin API's `addTileLayer` and accepts only `https://earthengine.googleapis.com` or `earthengine-highvolume` tile URLs. GeoLibre's own notebook bridge has no tile layer command and ignores messages from any frame but its Notebook panel iframe. Tested in the live prototype-tools Notebook panel with a stand-in `ee`, where 18 Earth Engine layers and 2 boundary rings appeared in the Layers panel. Matters because the notebook's connect cell depends on this handle, and a notebook listener now filters requests by a per-notebook token so two GeoLibre tabs do not both answer.

2026-09-13: GeoAgent's "Using latest model" note reads `defaultModel` from maplibre-gl-geoagent 0.5.4 (pinned by GeoLibre v1.9.0), not GeoLibre's own assistant list in `apps/geolibre-desktop/src/lib/assistant/provider.ts`, so `scripts/patch-geoagent-models.mjs` edits the package's dist in both deploys to `claude-opus-5`, adds Opus 5, Sonnet 5 and Fable 5.1 to its context window table, and keeps signed thinking blocks with empty text on replay. Verified on both live sites in `assets/maplibre-geoagent-*.js`; no chat with Opus 5 has been run through it. Matters because Opus 5 thinks by default with empty thinking text, which the unpatched formatter would drop from tool-call history.

2026-09-13: the startup project lists 77 corroboration layers from `scripts/corroboration_layers.py` in both repositories, typed `wms` with no `source.url`, because GeoLibre v1.9.0's `resolveProjectXyzLayers` fetches an `xyz` layer's address when it lacks `{x}{y}{z}` and logged 77 warnings for the bbox-templated ArcGIS export URLs. GeoLibre lists the last array entry at the top of the Layers panel. The Insect and Disease Survey export draws only below 1:250,000 even with a dynamicLayers override, so its per-year layers carry `minzoom` 11; the national view uses the TCA tree mortality image service. WFIGS perimeters, incident points and the interagency historic perimeters are FeatureServer only, with no drawing endpoint, and are not listed. Matters because every live layer was tile-tested on this date and those three remain undone.

2026-09-13: the three fire feature services are snapshotted at deploy by `scripts/fire_snapshots.py` into 23 PMTiles under `fire-records/` (295 MB published; 41,757 WFIGS perimeters, 425,502 incident points, 98,168 historic perimeters; 5.7 minutes on this Mac), cached per ISO week with actions/cache and listed as `pmtiles` layers carrying the metadata GeoLibre's Add Data writes (`sourceKind: pmtiles-url`, `externalNativeLayer`, `sourceLayers: ["fires"]`). Pages answers range requests with 206, and the 2025 perimeters drew nationally on the live site with no diagnostics. Matters because the first deploy of each week takes the download, and a failed download republishes the cached snapshot under its own date.

2026-09-14: the Land status folder adds BLM SMA cached tiles (xyz, z14), BIA AIAN LAR export, and snapshots of NPS boundaries and PAD-US 4.1 federal fee and proclamation layers, plus GRIP4 North America main roads (GP_RTP 1 to 4, z10, 41 MB) built once under a fixed cache key from the PBL fgdb. The weekly feature snapshot now takes 34 minutes on GitHub runners and publishes 487 MB beside a 277 MB app, about 764 MB of GitHub Pages' 1 GB site limit. With every layer on, headless Chrome logged 500 LANDFIRE fetch errors that vanished with an ordinary Chrome user agent, the USGS HeadlessChrome block again. Matters because the site has little room left under the Pages limit, and any headless check touching USGS must set a normal user agent.
